...
func _build_ui() -> void:
	...
	_output_label.text = "Output: (will be determined when exporting)"
	_output_label.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
	main_vbox.add_child(_output_label)
...
func _populate_tree_folder_mode() -> void:
	...
	var folder_icon = theme.get_icon("Folder", "EditorIcons")
...
func _do_export(output_path: String, package_name: String, files: Array[Dictionary], input_actions: Array[Dictionary], autoloads: Array[Dictionary], format: int) -> void:
	...
	for file_info in files:
		manifest["files"].append(file_info["rel_path"])
